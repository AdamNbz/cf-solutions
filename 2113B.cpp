...
void sol()
{
    int w, h, a, b, x1, y1, x2, y2; cin >> w >> h >> a >> b >> x1 >> y1 >> x2 >> y2;
    
    if (x1==x2) cout << (abs(y1-y2)%b ? "NO" : "YES") << el;
    else if (y1==y2) cout << (abs(x1-x2)%a ? "NO" : "YES") << el;
    else cout << ((x1-x2)%a==0 || (y1-y2)%b==0 ? "YES" : "NO") << el;
}
