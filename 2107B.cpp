...
void sol()
{
    ...
    ll s = accumulate(all(a), 0LL), mx = *max_element(all(a)), mn = *min_element(all(a)), diff = mx-mn;

    int cntmx = 0;
    for (auto x: a) cntmx += (x == mx);

    bool tom = 0;
    if (diff >= k+2) tom = 0;
    else if (diff == k+1 && cntmx > 1) tom = 0;
    else tom = (s & 1ll);
    
    cout << (tom ? "Tom" : "Jerry") << el; 
}
