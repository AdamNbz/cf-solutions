...
{
    int n; cin >> n;
    p64 p, q; cin >> p.fi >> p.se >> q.fi >> q.se;
    v64 a(n); for (auto &x: a) cin >> x;

    ll s = accumulate(all(a), 0LL), mx = *max_element(all(a));
    ll l = max(0ll, 2*mx-s), r = s, d = pow(q.fi-p.fi, 2) + pow(q.se-p.se, 2);
    cout << (l*l <= d && d <= r*r ? "YES" : "NO") << el;
}
