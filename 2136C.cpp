...
void sol()
{
    ...
    vv32 pos(n+1); for (int i=0; i<n; i++) pos[a[i]].pb(i);
    vector<pair<int, p32>> s;
    for (int i=1; i<=n; i++) 
    {
        int m = sz(pos[i]);
        for (int j=i; j<=m; j++) s.pb({pos[i][j-i], {pos[i][j-1], i}});
    }
    ...
    int m = sz(s);
    sort(all(s), [](pair<int, p32> x, pair<int, p32> y)
    {
        if (x.se.fi != y.se.fi) return x.se.fi < y.se.fi;
        return x.fi < y.fi;
    });
    v32 e(m), st(m), w(m);
    for (int i=0; i<m; i++) st[i] = s[i].fi, e[i] = s[i].se.fi, w[i] = s[i].se.se;
    
    v32 dp(m+1, 0);
    for (int i=1; i<=m; i++)
    {
        auto j = lower_bound(all(e), st[i-1])-e.begin();
